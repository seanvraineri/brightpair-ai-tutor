...
export default function LessonViewer(
  { lesson, onComplete }: LessonViewerProps,
) {
  ...
  const handleSubmitQuiz = () => {
    ...
    // Calculate score
    const quizSection = currentContent as QuizSection;
  ...
  const renderMathContent = (text: string) => {
    ...
    const parts = text.split(/(\$\$[\s\S]*?\$\$)|(\$[^\$]*?\$)/g).filter(
      Boolean,
    );
    return (
      <>
    ...
    if (question.type === "mcq") {
      return (
        <div key={questionId} className="mb-6">
          <p className="font-medium mb-3 quiz-stem">
            {index + 1}. {renderMathContent(question.stem)}
          </p>
            ...
            <div className="space-y-2">
              {question.choices?.map((choice: string, choiceIndex: number) => (
                ...
                >
                  ...
                  />
                  <Label
                    htmlFor={`q${questionId}-${choiceIndex}`}
                    className="flex-1 cursor-pointer quiz-choice"
                  >
              ...
              <p className="font-medium">
                {answers[questionId] === question.answer
                  ...
                  : `✗ Incorrect. The correct answer is: ${
                    question.choices?.find((c) => c === question.answer)
                      ?.toString()
                  }`}
              </p>
            </div>
      ...
      return (
        <div key={questionId} className="mb-6">
          ...
          {quizSubmitted && (
            <div
              className={`mt-3 p-3 rounded-md ${
                answers[questionId]?.trim().toLowerCase() ===
                    question.answer.trim().toLowerCase()
                  ? "bg-green-50"
                  : "bg-red-50"
              }`}
          ...
          {quizSubmitted && (
            <div className="mt-6 text-center">
              ...
              <p className="text-gray-600">
                You answered{" "}
                {Math.round((score / 100) * quizSection.questions.length)}{" "}
                out of {quizSection.questions.length} questions correctly.
              </p>
            </div>
          )}
