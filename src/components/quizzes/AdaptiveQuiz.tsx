...
import { Quiz, QuizQuestion, submitQuizAnswer, updateSkillMastery } from "@/services/quizService";
...
const AdaptiveQuiz: React.FC<AdaptiveQuizProps> = ({ quiz, onQuizComplete }) => {
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [userAnswers, setUserAnswers] = useState<string[]>(new Array(quiz.quiz.length).fill(''));
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      if (user && (user as any)?.id && result.tool_calls && result.tool_calls.length > 0) {
  ...
  const handleNext = () => {
    ...
    if (currentQuestionIndex < quiz.quiz.length - 1) {
      ...
      toast({
        ...
        description: `You got ${correctAnswersCount} out of ${quiz.quiz.length} questions correct.`,
      });
    }
  };
            ...
            {question.choices.map((choice, idx) => (
              <div 
                ...
                className={`flex items-center space-x-2 p-3 rounded-md border 
                  ${answerFeedback ? 
                    idx.toString() === question.answer ? 'bg-green-50 border-green-200' : 
                    userAnswers[currentQuestionIndex] === idx.toString() && !answerFeedback.isCorrect ? 'bg-red-50 border-red-200' : 
                    'border-gray-200' : 
                    userAnswers[currentQuestionIndex] === idx.toString() ? 'border-brightpair bg-brightpair-50' : 'border-gray-200'
                  }`}
              >
                <RadioGroupItem 
                  value={idx.toString()} 
        ...
        {question.type === 'latex' && (
          <div className="mb-4">
            ...
            <div className="mt-2 p-2 border rounded-md bg-gray-50">
              ...
              <PrettyMath latex={userAnswers[currentQuestionIndex] || '\\text{Your answer will appear here}'} />
            </div>
          </div>
        )}
  ...
  const renderFeedback = () => {
    ...
    return (
      <div className={`p-4 mb-4 rounded-md ${answerFeedback.isCorrect ? 'bg-green-50' : 'bg-red-50'}`}>
        <div className="flex items-start">
          {answerFeedback.isCorrect ? (
            <CheckCircle className="text-green-600 mt-1 mr-2 flex-shrink-0" size={20} />
          ) : (
            <AlertCircle className="text-red-600 mt-1 mr-2 flex-shrink-0" size={20} />
          )}
          <div>
            ...
            {!answerFeedback.isCorrect && currentQuestion.type === 'mcq' && (
              <p className="text-sm mt-2 font-medium">
                Correct answer: <PrettyMath latex={currentQuestion.choices?.[parseInt(currentQuestion.answer)] || ''} />
              </p>
            )}
            {!answerFeedback.isCorrect && currentQuestion.type !== 'mcq' && (
              <p className="text-sm mt-2 font-medium">
        ...
        <CardContent>
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600">
              You answered {correctAnswersCount} out of {quiz.quiz.length} questions correctly.
            </p>
          </div>
          
          <div className="mb-6">
            <Progress value={(correctAnswersCount / quiz.quiz.length) * 100} className="h-3" />
          </div>
          ...
          <div className="bg-brightpair-50 p-4 rounded-md mb-4">
            ...
            <p className="text-sm">
              Your mastery of these skills has been updated based on your quiz performance.
              Continue practicing to further improve your understanding.
            </p>
          </div>
          
          <div className="mt-6">
            <Button onClick={onQuizComplete} className="w-full bg-brightpair hover:bg-brightpair-600">
              Return to Quizzes
            </Button>
          </div>
        </CardContent>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-sm font-medium px-2 py-1 bg-gray-100 rounded">
            {currentQuestion.difficulty === 'easy' ? 'Easy' : 
             currentQuestion.difficulty === 'med' ? 'Medium' : 'Hard'}
          </div>
        </div>
      </CardHeader>
      <CardContent>
