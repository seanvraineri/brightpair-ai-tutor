...
const MOCK_SHARE_SETTINGS: ReportShareSetting[] = [
  {
    id: 'share-1',
    parent_id: 'parent-1',
    is_shared: true,
    ...
    auto_expire_at: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString()
  },
  {
    id: 'share-2',
    parent_id: 'parent-2',
...
const ReportShareOptions: React.FC<ReportShareOptionsProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setShareSettings(MOCK_SHARE_SETTINGS);
  ...
  const toggleParentSharing = (parentId: string, isShared: boolean) => {
    setShareSettings(prev => {
      ...
      const existingSettingIndex = prev.findIndex(setting => setting.parent_id === parentId);
  ...
  const getViewStatus = (setting: ReportShareSetting): React.ReactNode => {
    if (!setting.is_shared) {
      return <Badge variant="outline" className="bg-gray-100 text-gray-800">Not Shared</Badge>;
    }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Manage how the report "{reportType}" for {studentName} from {formatDate(reportDate)} is shared with parents
        </CardDescription>
      </CardHeader>
          ...
          <>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-gray-500">
                  {shareSettings.filter(s => s.is_shared).length} of {parents.length} parents have access
                </span>
              </div>
              
              <div className="border rounded-md divide-y">
                {parents.map(parent => {
                  const setting = shareSettings.find(s => s.parent_id === parent.id);
                  ...
                  return (
                    <div key={parent.id} className="flex items-center justify-between p-4">
                      <div className="flex items-center space-x-3">
                        <Switch
                          checked={isShared}
                          onCheckedChange={(checked) => toggleParentSharing(parent.id, checked)}
                        />
                        <div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                This link can be shared with anyone who needs access to this report.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">
                After this period, parents will no longer be able to access the report.
              </p>
            </div>
