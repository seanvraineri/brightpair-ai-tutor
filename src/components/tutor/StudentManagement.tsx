...
const StudentManagement: React.FC = () => {
  ...
  const filteredStudents = mockStudents.filter(student => {
    ...
    const matchesGrade = !filterGrade || filterGrade === "all" || student.grade === filterGrade;
    const matchesSubject = !filterSubject || filterSubject === "all" || student.subjects.includes(filterSubject);
  ...
  const getPendingAssignmentsForStudent = (studentId: string) => {
    return mockAssignments.filter(assignment => assignment.studentId === studentId);
  };
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-col md:flex-row gap-4">
                ...
                {(searchQuery || filterGrade || filterSubject) && (
                  <Button variant="outline" onClick={handleResetFilters} className="flex-shrink-0">
          ...
          {selectedStudent && (
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  <CardTitle>Assignments for {getStudentById(selectedStudent)?.name}</CardTitle>
              ...
              <CardContent className="space-y-6">
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          ...
                          <SelectContent>
                            {getStudentById(selectedStudent)?.subjects.map(subject => (
                              <SelectItem key={subject} value={subject}>{subject}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                  ...
                  {getPendingAssignmentsForStudent(selectedStudent).length === 0 ? (
                    <div className="text-gray-500 text-center py-4">
                      No pending assignments
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {getPendingAssignmentsForStudent(selectedStudent).map(assignment => (
                            ...
                            <div className="flex justify-between items-center">
                              <div>
                                ...
                                <div className="flex items-center text-sm text-gray-500 mt-1">
                                  <Badge variant="outline" className="mr-2">{assignment.subject}</Badge>
      ...
      <Dialog open={isAddStudentDialogOpen} onOpenChange={setIsAddStudentDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the details of the student you want to add to your management dashboard.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                {newStudent.subjects.map(subject => (
                  <Badge key={subject} className="flex items-center gap-1 px-3 py-1">
                    {subject}
                    <X 
                      size={14} 
