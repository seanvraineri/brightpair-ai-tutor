...
import { Loader2, Sparkles, Lightbulb, Info, Send, RefreshCw, UserPlus, Save } from 'lucide-react';
...
const MOCK_AI_ENHANCE = (originalMessage: string, tone: string): Promise<AIEnhancement> => {
    ...
    setTimeout(() => {
      const enhancements: Record<string, AIEnhancement> = {
        'professional': {
          ...
          tone: 'professional',
          explanation: 'Enhanced with more formal language and professional tone appropriate for academic progress updates. Added specific details about the student\'s achievements and clear next steps.'
        },
        'friendly': {
          original: originalMessage,
          enhanced: originalMessage.replace(
          ...
          tone: 'friendly',
          explanation: 'Made the message warmer and more conversational while maintaining professionalism. Used more approachable language and added encouraging phrases.'
        },
        'casual': {
          original: originalMessage,
          enhanced: originalMessage.replace(
          ...
          tone: 'casual',
          explanation: 'Transformed the message to be more casual and conversational. Used shorter sentences and more everyday language while still covering all the important points.'
        }
      };
      
      resolve(enhancements[tone] || {
        original: originalMessage,
...
const MessageComposer: React.FC<MessageComposerProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (studentId) {
          setParents(MOCK_PARENTS.filter(parent => parent.student_id === studentId));
        } else {
  ...
  useEffect(() => {
    if (selectedStudentId) {
      setParents(MOCK_PARENTS.filter(parent => parent.student_id === selectedStudentId));
      
      // If there's only one parent, select them automatically
      const filteredParents = MOCK_PARENTS.filter(parent => parent.student_id === selectedStudentId);
  ...
  useEffect(() => {
    if (selectedTemplate) {
      ...
      if (template) {
        ...
        if (selectedParent) {
          populatedContent = populatedContent.replace(/\[parent_name\]/g, selectedParent.full_name);
        }
        ...
        if (!subject) {
          setSubject(template.title + ` for ${selectedStudent?.full_name || ''}`);
        }
      }
    }
  }, [selectedTemplate, selectedStudentId, selectedParentId, subject, students, parents, templates]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      console.log('Sending message:', {
        ...
        subject,
        message: usesEnhanced && aiEnhancement ? aiEnhancement.enhanced : message
      });
    ...
    <Card className="w-full">
            ...
            <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  disabled={!selectedStudentId || !!parentId || parents.length === 0}
                >
            ...
            <div className="border rounded-lg p-4 bg-gray-50">
              ...
              <Tabs defaultValue="templates" className="w-full">
                <TabsList className="w-full">
                  ...
                  <TabsTrigger value="tone" className="flex-1">Tone & Style</TabsTrigger>
                  {aiEnhancement && (
                    <TabsTrigger value="ai-suggestions" className="flex-1">
                      AI Suggestions
                      <Badge className="ml-2 bg-brightpair text-white">New</Badge>
                    </TabsTrigger>
                  )}
                </TabsList>
                ...
                <TabsContent value="templates" className="py-2">
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      ...
                      <Button variant="outline" className="justify-start h-auto py-2 px-3 border-dashed">
                        <div className="flex items-center space-x-2">
                    ...
                    {useAI && (
                      <div className="flex justify-end">
                        <Button 
                          ...
                          disabled={enhancingMessage || !message || !selectedTone}
                  ...
                  <TabsContent value="ai-suggestions" className="py-2">
                    <div className="space-y-4">
                      <div className="flex items-start space-x-2 text-sm bg-amber-50 border border-amber-200 rounded-md p-3">
                        ...
                        <div>
                          <p className="font-medium text-amber-800">AI Enhancement Explanation</p>
                          <p className="text-gray-600 mt-1">{aiEnhancement.explanation}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <Button
                          ...
                          onClick={() => setUsesEnhanced(true)}
                          className={usesEnhanced ? "bg-brightpair hover:bg-brightpair-600" : ""}
                        >
                          Use Enhanced
                        </Button>
                        <Button
                        ...
                        >
                          <RefreshCw className={`h-4 w-4 mr-2 ${enhancingMessage ? 'animate-spin' : ''}`} />
                          Regenerate
                        </Button>
                      </div>
                    </div>
      ...
      <CardFooter className="flex justify-between">
        ...
        <div className="flex space-x-2">
          ...
          <Button 
            onClick={sendMessage}
            disabled={sendingMessage || !selectedParentId || !subject || !message}
