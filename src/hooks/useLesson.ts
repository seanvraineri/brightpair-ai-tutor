...
const mockLesson: Lesson = {
  ...
  sections: [
    ...
    {
      type: "quiz",
      content_md: "Test your understanding of the Chain Rule with these practice questions:",
      questions: [
        {
          id: "q1",
          type: "mcq",
...
interface DbFunctions {
  buildStudentSnapshot: (params: { p_student: string }) => Promise<any>;
  getPassagesForSkill: (params: { p_student: string, p_skill: string }) => Promise<any[]>;
  getRecentErrors: (params: { p_student: string, p_skill: string }) => Promise<any[]>;
  updateStudentSkill: (params: { p_student: string, p_skill: string, p_delta: number }) => Promise<void>;
}
...
export const useLesson = (studentId: string, skillId: string) =>
  useQuery({
    ...
    queryFn: async () => {
      console.log(`Generating lesson for student: ${studentId}, skill: ${skillId}`);
