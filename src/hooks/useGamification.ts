...
import { User, Achievement, Badge } from '@/contexts/UserTypes';
...
const ACHIEVEMENT_CRITERIA: AchievementCriteria = {
  ...
  'knowledge-seeker': { activityType: ActivityType.COMPLETE_LESSON, threshold: 5 },
  'homework-hero': { activityType: ActivityType.COMPLETE_HOMEWORK, threshold: 10 },
  'flashcard-fanatic': { activityType: ActivityType.PRACTICE_FLASHCARDS, threshold: 20 },
  'dedicated-learner': { activityType: null, threshold: 500, requiresStreak: false }, // Based on total XP
  'consistent-learner': { activityType: null, threshold: 7, requiresStreak: true } // Based on streak
};
...
export const useGamification = (setUser: React.Dispatch<React.SetStateAction<User | null>>) => {
  ...
  const trackActivity = async (userId: string, activityType: ActivityType, details?: any) => {
  ...
  const checkAchievements = (activityType: ActivityType) => {
    setUser(prev => {
      ...
      Object.entries(ACHIEVEMENT_CRITERIA).forEach(([achievementId, criteria]) => {
        // Find the achievement in user's list
        const achievementIndex = updatedAchievements.findIndex(a => a.id === achievementId);
        if (achievementIndex === -1 || updatedAchievements[achievementIndex].isUnlocked) return;
        
        // Check if this activity type matches the criteria
        if (criteria.activityType !== null && criteria.activityType !== activityType) return;
  ...
  const earnXP = (amount: number) => {
    setUser(prev => {
      ...
      if (currentXP >= xpToNextLevel) {
        ...
        if (newLevel >= 5) {
          const bronzeBadgeIndex = updatedBadges.findIndex(b => b.id === 'bronze-scholar');
          if (bronzeBadgeIndex !== -1 && !updatedBadges[bronzeBadgeIndex].isUnlocked) {
            updatedBadges[bronzeBadgeIndex].isUnlocked = true;
          }
        }
        if (newLevel >= 10) {
          const silverBadgeIndex = updatedBadges.findIndex(b => b.id === 'silver-scholar');
          if (silverBadgeIndex !== -1 && !updatedBadges[silverBadgeIndex].isUnlocked) {
            updatedBadges[silverBadgeIndex].isUnlocked = true;
          }
        }
        if (newLevel >= 20) {
          const goldBadgeIndex = updatedBadges.findIndex(b => b.id === 'gold-scholar');
  ...
  const calculateSubjectProgress = (subjectId: string): number => {
    ...
    const subjectActivities = activityLog.filter(a => a.details?.subjectId === subjectId);
    
    // Calculate total completed lessons/quizzes
    const completedLessons = subjectActivities.filter(a => 
      a.type === ActivityType.COMPLETE_LESSON
    ).length;
    
    const completedQuizzes = subjectActivities.filter(a => 
      a.type === ActivityType.COMPLETE_QUIZ
    ).length;
    ...
    const percentage = Math.min(
      100, 
      Math.round(((completedLessons + completedQuizzes) / totalActivitiesPerSubject) * 100)
    );
