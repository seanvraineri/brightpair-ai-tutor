...
const studentProgressData = [
  { name: 'Alex', score: 85, ageGroupAverage: 76, subject: 'Mathematics', grade: '10th' },
  { name: 'Jamie', score: 92, ageGroupAverage: 81, subject: 'English', grade: '11th' },
  { name: 'Taylor', score: 78, ageGroupAverage: 74, subject: 'Science', grade: '9th' },
  { name: 'Jordan', score: 90, ageGroupAverage: 83, subject: 'Physics', grade: '12th' },
];
...
const sessionAnalyticsData = [
  { 
    ...
    notes: 'Alex showed good progress with linear equations but needs more practice with function notation.'
  },
  { 
    id: 'sess2', 
    date: '2023-06-16', 
    ...
    objectives: ['Develop strong thesis statements', 'Properly format citations'],
    objectivesAchieved: 2,
    studentEngagement: 5,
    studentComprehension: 4,
    followupRequired: false,
    notes: 'Jamie has shown excellent progress in developing strong thesis statements and properly formatting citations.'
  },
  { 
    id: 'sess3', 
    date: '2023-06-15', 
    ...
    followupRequired: true,
    notes: 'Taylor struggles with distinguishing between mitosis and meiosis. Needs additional visual aids and practice.'
  },
  { 
    id: 'sess4', 
    date: '2023-06-15', 
    ...
    objectives: ['Solve projectile motion problems', 'Calculate vector components'],
    objectivesAchieved: 2,
    studentEngagement: 4,
    studentComprehension: 4,
    followupRequired: false,
    notes: 'Jordan demonstrated strong understanding of vectors and projectile motion. Ready to move on to forces.'
  },
  { 
    id: 'sess5', 
    date: '2023-06-14', 
    ...
    studentComprehension: 4,
    followupRequired: false,
    notes: 'Alex has mastered the Pythagorean theorem and can confidently solve triangle problems.'
  }
];
...
const assignmentsData = [
  { 
    ...
    status: 'completed',
    score: 85,
    feedback: 'Good work on solving equations. Keep practicing with the more complex examples.',
  ...
  { 
    ...
    status: 'completed',
    score: 92,
    feedback: 'Excellent analysis of the themes. Your writing style has improved significantly.',
  ...
  { 
    ...
    status: 'completed',
    score: 76,
    feedback: 'You understood the basic concepts but struggled with cell division. Review mitosis and meiosis.',
...
const Reports: React.FC = () => {
  ...
  const calculateSessionMetrics = () => {
    ...
    const totalHours = sessionAnalyticsData.reduce((acc, session) => acc + session.duration, 0) / 60;
    const averageDuration = totalHours / totalSessions;
    const objectivesAchieved = sessionAnalyticsData.reduce((acc, session) => acc + session.objectivesAchieved, 0);
    const totalObjectives = sessionAnalyticsData.reduce((acc, session) => acc + session.objectives.length, 0);
    const objectivesAchievedRate = Math.round((objectivesAchieved / totalObjectives) * 100);
    
    const averageEngagement = sessionAnalyticsData.reduce((acc, session) => acc + session.studentEngagement, 0) / totalSessions;
    const averageComprehension = sessionAnalyticsData.reduce((acc, session) => acc + session.studentComprehension, 0) / totalSessions;
  ...
  const calculateAssignmentMetrics = () => {
    ...
    const scores = assignmentsData.filter(a => a.score !== null).map(a => a.score as number);
    const averageScore = scores.length > 0 
      ? Math.round(scores.reduce((acc, score) => acc + score, 0) / scores.length) 
      : 0;
    
    const onTimeSubmissions = assignmentsData.filter(a => 
      a.submittedDate && a.dueDate && new Date(a.submittedDate) <= new Date(a.dueDate)
    ).length;
  ...
  const calculateReferralMetrics = () => {
    ...
    const totalCommission = referralData.reduce((acc, referral) => acc + referral.totalEarned, 0);
    const monthlyCommission = monthlyCommissionData[monthlyCommissionData.length - 1].commission;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-600">Monitor student progress and teaching effectiveness</p>
          </div>
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => exportPdf(reportData || {})}>
              <Download className="h-4 w-4 mr-2" />
              PDF
            </Button>
            <Button variant="outline" onClick={() => exportCsv([
              ['Active Students', reportData?.active_students?.toString() || 'N/A'],
              ['Hours', reportData?.hours ? Math.round(reportData.hours/3600).toString() : 'N/A'],
              ['Avg Score', reportData?.avg_score ? Math.round(reportData.avg_score).toString() : 'N/A']
            ])}>
              <Download className="h-4 w-4 mr-2" />
              CSV
            </Button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    How each student performs compared to their age group in the same subject
                  </CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      >
                        ...
                        <Tooltip 
                          formatter={(value, name) => [
                            `${value}%`, 
                            name === 'score' ? 'Student Score' : 'Age Group Average'
                          ]}
                          labelFormatter={(name) => {
                            const student = studentProgressData.find(s => s.name === name);
                            return `${student?.subject} (${student?.grade})`;
                          }}
                        />
                        <Legend />
                        <Bar dataKey="score" name="Student Score" fill="#4D8BF9" />
                        <Bar dataKey="ageGroupAverage" name="Age Group Average" fill="#82ca9d" />
                        <ReferenceLine x={75} stroke="#ff7300" label="Minimum Target" />
                      </RechartsBarChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
                  ...
                  <div className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        <YAxis />
                        <Tooltip />
                        <Line type="monotone" dataKey="hours" name="Hours" stroke="#8884d8" activeDot={{ r: 8 }} />
                      </RechartsLineChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="border-l-2 border-green-500 pl-4 pb-4">
                      ...
                      <p className="font-medium">Jamie Johnson submitted homework</p>
                    ...
                    <div className="border-l-2 border-orange-500 pl-4 pb-4">
                      ...
                      <p className="font-medium">New student assigned: Jordan Lee</p>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1 mt-4">
                      {studentDetailedData.map(student => (
                        <div 
                          key={student.id} 
                          className={`cursor-pointer p-3 rounded-md border ${student.id === selectedStudent ? 'bg-brightpair/5 border-brightpair' : 'hover:bg-gray-50 border-gray-200'}`}
                          onClick={() => setSelectedStudent(student.id)}
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              <h3 className="font-medium">{student.name}</h3>
                              <p className="text-sm text-gray-500">{student.grade} • {student.subjects.join(', ')}</p>
                            </div>
                            <Badge className={student.improvement > 15 ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}>
                  ...
                  <>
                    <Card className="mb-6">
                      <CardHeader>
                        <div className="flex justify-between items-start">
                          <div>
                            <CardTitle>{getStudentById(selectedStudent)?.name} - Progress Summary</CardTitle>
                            <CardDescription>
                              Overall academic performance and engagement metrics
                            </CardDescription>
                          </div>
                          <div className="flex gap-2">
                      ...
                      <CardContent>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                          <div className="bg-gray-50 p-4 rounded">
                            <p className="text-sm text-gray-500">Academic Improvement</p>
                            <div className="flex items-center mt-1">
                              <p className="text-2xl font-bold mr-1">+{getStudentById(selectedStudent)?.improvement}%</p>
                              {renderTrendIndicator(getStudentById(selectedStudent)?.improvement || 0)}
                            </div>
                            <p className="text-xs text-gray-500 mt-1">Since first assessment</p>
                          </div>
                          
                          <div className="bg-gray-50 p-4 rounded">
                            <p className="text-sm text-gray-500">Attendance Rate</p>
                            <div className="flex items-center mt-1">
                              <p className="text-2xl font-bold mr-1">{getStudentById(selectedStudent)?.attendanceRate}%</p>
                              {renderTrendIndicator(getStudentById(selectedStudent)?.attendanceRate || 0 - 90)}
                            </div>
                          ...
                          <div className="bg-gray-50 p-4 rounded">
                            ...
                            <div className="flex items-center mt-1">
                              <p className="text-2xl font-bold mr-1">{getStudentById(selectedStudent)?.homeworkCompletion}%</p>
                              {renderTrendIndicator(getStudentById(selectedStudent)?.homeworkCompletion || 0 - 80)}
                            </div>
                            <p className="text-xs text-gray-500 mt-1">Assignments completed</p>
                          </div>
                          
                          <div className="bg-gray-50 p-4 rounded">
                            <p className="text-sm text-gray-500">Engagement Score</p>
                            <div className="flex items-center mt-1">
                              <p className="text-2xl font-bold mr-1">{getStudentById(selectedStudent)?.engagementScore}/5</p>
                              {renderTrendIndicator((getStudentById(selectedStudent)?.engagementScore || 0) - 3.5)}
                            </div>
                            <p className="text-xs text-gray-500 mt-1">Class participation</p>
                          </div>
                        </div>
                        
                        <div className="mt-4">
                          <h3 className="text-md font-medium mb-3">Academic Progress Timeline</h3>
                          <div className="h-[250px]">
                            <ResponsiveContainer width="100%" height="100%">
                              <RechartsLineChart
                                data={[
                                  { month: 'Jan', score: getStudentById(selectedStudent)?.initialAssessment || 0 },
                                  { month: 'Feb', score: getStudentById(selectedStudent)?.initialAssessment ? getStudentById(selectedStudent)?.initialAssessment + 5 : 0 },
                                  { month: 'Mar', score: getStudentById(selectedStudent)?.initialAssessment ? getStudentById(selectedStudent)?.initialAssessment + 7 : 0 },
                                  { month: 'Apr', score: getStudentById(selectedStudent)?.initialAssessment ? getStudentById(selectedStudent)?.initialAssessment + 10 : 0 },
                                  { month: 'May', score: getStudentById(selectedStudent)?.initialAssessment ? getStudentById(selectedStudent)?.initialAssessment + 11 : 0 },
                                  { month: 'Jun', score: getStudentById(selectedStudent)?.currentScore || 0 },
                                ]}
                                margin={{ top: 20, right: 30, left: 20, bottom: 5 }}
                              >
                                <CartesianGrid strokeDasharray="3 3" />
                                ...
                                <YAxis domain={[0, 100]} />
                                <Tooltip />
                                <Line type="monotone" dataKey="score" name="Score" stroke="#4D8BF9" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 8 }} />
                              </RechartsLineChart>
                            </ResponsiveContainer>
                          </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <Card>
                        ...
                        <CardContent>
                          <div className="space-y-4">
                            <div>
                              <h3 className="text-sm font-semibold text-gray-600 mb-2">Areas of Strength</h3>
                              <div className="flex flex-wrap gap-2">
                                {getStudentById(selectedStudent)?.strengthAreas.map(area => (
                            ...
                            <div>
                              <h3 className="text-sm font-semibold text-gray-600 mb-2">Areas for Improvement</h3>
                              <div className="flex flex-wrap gap-2">
                                {getStudentById(selectedStudent)?.improvementAreas.map(area => (
                                  <Badge key={area} className="bg-orange-100 text-orange-800">{area}</Badge>
                                ))}
                              </div>
                            </div>
                            
                            <div>
                              <h3 className="text-sm font-semibold text-gray-600 mb-2">Learning Style</h3>
                              <Badge className="bg-blue-100 text-blue-800">{getStudentById(selectedStudent)?.learningStyle}</Badge>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
                          ...
                          <div className="h-[250px]">
                            <ResponsiveContainer width="100%" height="100%">
                              ...
                              >
                                ...
                                <Tooltip />
                                <Legend />
                                <Bar dataKey="beforeScore" name="Initial Assessment" fill="#8884d8" />
                                <Bar dataKey="afterScore" name="Current Level" fill="#4D8BF9" />
                              </RechartsBarChart>
                            </ResponsiveContainer>
                          </div>
                        </CardContent>
                  ...
                  <Card className="h-full flex items-center justify-center">
                    <CardContent className="text-center py-12">
                      ...
                      <p className="text-gray-500 mb-4 max-w-md mx-auto">
                        Select a student from the list to view their detailed progress metrics and academic growth.
                      </p>
                    </CardContent>
                  </Card>
                )}
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div className="bg-gray-50 p-4 rounded-md">
                        ...
                        <p className="text-2xl font-bold">{sessionMetrics.totalHours}h</p>
                      </div>
                      
                      <div className="bg-gray-50 p-4 rounded-md">
                        <p className="text-sm font-medium text-gray-500">Average Session Length</p>
                        <p className="text-2xl font-bold">{sessionMetrics.averageDuration}h</p>
                      </div>
                      
                      <div className="bg-gray-50 p-4 rounded-md">
                        <p className="text-sm font-medium text-gray-500">Objectives Achieved</p>
                        <p className="text-2xl font-bold">{sessionMetrics.objectivesAchievedRate}%</p>
                      </div>
                      
                      <div className="bg-gray-50 p-4 rounded-md">
                        <p className="text-sm font-medium text-gray-500">Average Engagement Score</p>
                        <p className="text-2xl font-bold">{sessionMetrics.averageEngagement}/5</p>
                      </div>
                      
                      <div className="bg-gray-50 p-4 rounded-md">
                        <p className="text-sm font-medium text-gray-500">Average Comprehension Score</p>
                        <p className="text-2xl font-bold">{sessionMetrics.averageComprehension}/5</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <RechartsPieChart>
                              <Pie
                                ...
                                label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}
                              >
                                {sessionEffectivenessData.map((entry, index) => (
                                  <Cell key={`cell-${index}`} fill={index === 0 ? '#4D8BF9' : index === 1 ? '#82ca9d' : '#ff8042'} />
                                ))}
                              </Pie>
                              <Tooltip />
                            </RechartsPieChart>
                        ...
                        <div className="h-[250px]">
                          <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                              ...
                              <YAxis />
                              <Tooltip />
                              <Line type="monotone" dataKey="hours" name="Hours" stroke="#4D8BF9" strokeWidth={2} />
                            </RechartsLineChart>
                          </ResponsiveContainer>
                        </div>
                      </CardContent>
                  ...
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-2">
                        <Select defaultValue={sessionsDateRange} onValueChange={setSessionsDateRange}>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {sessionAnalyticsData.map((session) => (
                          <span key={session.id} style={{display: 'contents'}}>
                            <TableRow>
                              ...
                              <TableCell>
                                <div className="flex">
                                  {Array(5).fill(0).map((_, i) => (
                                    <div 
                                      key={i} 
                                      className={`h-2 w-2 rounded-md mx-0.5 ${i < session.studentEngagement ? 'bg-green-500' : 'bg-gray-200'}`} 
                                    />
                                  ))}
                                </div>
                              </TableCell>
                              <TableCell>
                                <div className="flex">
                                  {Array(5).fill(0).map((_, i) => (
                                    <div 
                                      key={i} 
                                      className={`h-2 w-2 rounded-md mx-0.5 ${i < session.studentComprehension ? 'bg-blue-500' : 'bg-gray-200'}`} 
                                    />
                                  ))}
                                </div>
                              </TableCell>
                              <TableCell>
                                {session.followupRequired ? (
                                  <Badge className="bg-amber-100 text-amber-800">Needed</Badge>
                                ) : (
                              ...
                              <TableCell>
                                ...
                                >
                                  {session.id === selectedSession ? <ArrowUp className="h-4 w-4" /> : <ArrowDown className="h-4 w-4" />}
                                </Button>
                              </TableCell>
                            </TableRow>
                            {session.id === selectedSession && (
                              <TableRow>
                                <TableCell colSpan={8} className="bg-gray-50">
                                  <div className="p-2">
                                    <div className="mb-3">
                                      <h4 className="text-sm font-semibold mb-1">Topics Covered</h4>
                                      <div className="flex flex-wrap gap-1">
                                        {session.topicsCovered.map((topic) => (
                                          <Badge key={topic} className="bg-blue-100 text-blue-800">{topic}</Badge>
                                        ))}
                                      </div>
                                    </div>
                                    <div className="mb-3">
                                      ...
                                      <ul className="list-disc pl-5 text-sm">
                                        {session.objectives.map((objective, index) => (
                                          <li key={index} className={`${index < session.objectivesAchieved ? 'text-green-700' : 'text-gray-500'}`}>
                                            {objective} {index < session.objectivesAchieved && '✓'}
                                          </li>
                                        ))}
                                      </ul>
                                    </div>
                                    <div>
                                      <h4 className="text-sm font-semibold mb-1">Session Notes</h4>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="bg-gray-50 p-4 rounded-md">
                        ...
                        <p className="text-2xl font-bold">{assignmentMetrics.total}</p>
                      </div>
                      
                      <div className="bg-gray-50 p-4 rounded-md">
                        <p className="text-sm font-medium text-gray-500">Completion Rate</p>
                        <p className="text-2xl font-bold">{assignmentMetrics.completionRate}%</p>
                        <div className="w-full bg-gray-200 h-2 mt-2 rounded-md overflow-hidden">
                          <div 
                            className="bg-brightpair h-2 rounded" 
                            style={{ width: `${assignmentMetrics.completionRate}%` }}
                          ></div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-50 p-4 rounded-md">
                        ...
                        <p className="text-2xl font-bold">{assignmentMetrics.averageScore}%</p>
                        <div className="w-full bg-gray-200 h-2 mt-2 rounded-md overflow-hidden">
                          <div 
                            className={`h-2 rounded-md ${
                              assignmentMetrics.averageScore >= 90 ? 'bg-green-500' :
                              assignmentMetrics.averageScore >= 80 ? 'bg-blue-500' :
                              assignmentMetrics.averageScore >= 70 ? 'bg-yellow-500' : 'bg-red-500'
                            }`}
                            style={{ width: `${assignmentMetrics.averageScore}%` }}
                          ></div>
                        </div>
                      </div>
                      
                      <div className="bg-gray-50 p-4 rounded-md">
                        <p className="text-sm font-medium text-gray-500">On-Time Submission Rate</p>
                        <p className="text-2xl font-bold">{assignmentMetrics.onTimeRate}%</p>
                        <div className="w-full bg-gray-200 h-2 mt-2 rounded-md overflow-hidden">
                          <div 
                            className="bg-green-500 h-2 rounded" 
                            style={{ width: `${assignmentMetrics.onTimeRate}%` }}
                          ></div>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="h-[250px]">
                        <ResponsiveContainer width="100%" height="100%">
                          ...
                          >
                            ...
                            <YAxis domain={[0, 100]} />
                            <Tooltip />
                            <Bar dataKey="completionRate" name="Completion Rate %" fill="#4D8BF9" />
                          </RechartsBarChart>
                        </ResponsiveContainer>
                      </div>
                    </CardContent>
                              ...
                              >
                                {scoreDistribution.map((entry, index) => (
                                  <Cell 
                                    key={`cell-${index}`} 
                                    fill={
                                      index === 0 ? '#10b981' : // 90-100
                                      index === 1 ? '#3b82f6' : // 80-89
                                      index === 2 ? '#f59e0b' : // 70-79
                                      index === 3 ? '#f97316' : // 60-69
                                      '#ef4444'                 // Below 60
                                    } 
                                  />
                                ))}
                          ...
                          <div className="space-y-4">
                            <div>
                              <div className="flex justify-between text-sm mb-1">
                                <span>Completed</span>
                                <span>{assignmentMetrics.completed} of {assignmentMetrics.total}</span>
                              </div>
                              <div className="h-3 bg-gray-200 rounded">
                                <div 
                                  className="h-3 bg-green-500 rounded" 
                                  style={{ width: `${(assignmentMetrics.completed / assignmentMetrics.total) * 100}%` }}
                                ></div>
                              </div>
                            </div>
                            ...
                            <div>
                              <div className="flex justify-between text-sm mb-1">
                                <span>Pending</span>
                                <span>{assignmentsData.filter(a => a.status === 'pending').length} of {assignmentMetrics.total}</span>
                              </div>
                              <div className="h-3 bg-gray-200 rounded">
                                <div 
                                  className="h-3 bg-blue-500 rounded" 
                                  style={{ width: `${(assignmentsData.filter(a => a.status === 'pending').length / assignmentMetrics.total) * 100}%` }}
                                ></div>
                              </div>
                            </div>
                            ...
                            <div>
                              <div className="flex justify-between text-sm mb-1">
                                <span>Overdue</span>
                                <span>{assignmentsData.filter(a => a.status === 'overdue').length} of {assignmentMetrics.total}</span>
                              </div>
                              <div className="h-3 bg-gray-200 rounded">
                                <div 
                                  className="h-3 bg-red-500 rounded" 
                                  style={{ width: `${(assignmentsData.filter(a => a.status === 'overdue').length / assignmentMetrics.total) * 100}%` }}
                                ></div>
                              </div>
                            </div>
                          </div>
                ...
                <Card>
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-2">
                        <Select defaultValue={assignmentFilter} onValueChange={setAssignmentFilter}>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {assignmentsData
                          ...
                          .map(assignment => (
                          <span key={assignment.id} style={{display: 'contents'}}>
                            <TableRow>
                              <TableCell>
                              ...
                              <TableCell>
                                ...
                                >
                                  {assignment.id === selectedAssignment ? <ArrowUp className="h-4 w-4" /> : <ArrowDown className="h-4 w-4" />}
                                </Button>
                              </TableCell>
                            </TableRow>
                                ...
                                <TableCell colSpan={8} className="bg-gray-50">
                                  <div className="p-2 space-y-4">
                                    {assignment.status === 'completed' && (
                                      <>
                                        <div>
                                          <h4 className="text-sm font-semibold mb-2">Skills Assessment</h4>
                                          <div className="space-y-2">
                                            {assignment.skillsAssessed.map(skill => (
                                              <div key={skill.skill}>
                                                <div className="flex justify-between text-sm mb-1">
              ...
              <div className="lg:col-span-1">
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="bg-gray-50 p-4 rounded-md">
                        <p className="text-sm font-medium text-gray-500">This Month's Commission</p>
                        <p className="text-2xl font-bold text-green-600">${referralMetrics.monthlyCommission}</p>
                      </div>
                      
                      <div className="bg-gray-50 p-4 rounded-md">
                        <p className="text-sm font-medium text-gray-500">Total Earnings (All Time)</p>
                        <p className="text-2xl font-bold text-green-600">${referralMetrics.totalCommission}</p>
                      </div>
                      
                      <div className="bg-gray-50 p-4 rounded-md">
                        <p className="text-sm font-medium text-gray-500">Active Referrals</p>
                        <div className="flex items-center">
                          <p className="text-2xl font-bold">{referralMetrics.activeReferrals}/{referralMetrics.totalReferrals}</p>
                          <Badge className="ml-2 bg-green-100 text-green-800">{referralMetrics.activeRate}%</Badge>
                        </div>
                      </div>
                      
                      <div className="bg-gray-50 p-4 rounded-md">
                        <p className="text-sm font-medium text-gray-500">Pending Activations</p>
                        <p className="text-2xl font-bold">{referralMetrics.pendingReferrals}</p>
                      </div>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="space-y-4 p-4">
                          <div>
                            <h3 className="font-medium mb-2">Referral Commission Structure</h3>
                            <ul className="space-y-2 text-sm">
                              <li className="flex justify-between">
                                <span>Basic Plan ($99/month)</span>
                                <span className="font-medium">15% = $14.85</span>
                              </li>
                              <li className="flex justify-between">
                                <span>Standard Plan ($149/month)</span>
                                <span className="font-medium">15% = $22.35</span>
                              </li>
                              <li className="flex justify-between">
                                <span>Premium Plan ($249/month)</span>
                                <span className="font-medium">15% = $37.35</span>
                              </li>
                            </ul>
                          </div>
                          ...
                          <div className="pt-4 border-t border-gray-200">
                            ...
                            <p className="text-sm text-gray-600">
                              Commissions are paid on the 15th of each month for the previous month's active referrals. Payments are made via direct deposit to your registered bank account.
                            </p>
                          </div>
                        </div>
                      </CardContent>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {referralData.map(referral => (
                          <span key={referral.id} style={{display: 'contents'}}>
                            <TableRow>
                              <TableCell>{referral.clientName}</TableCell>
                              <TableCell>{referral.referralDate}</TableCell>
                              <TableCell>{referral.plan} (${referral.planValue}/mo)</TableCell>
                              <TableCell className="text-green-600">${referral.commission}</TableCell>
                              <TableCell className="text-green-600">${referral.totalEarned}</TableCell>
                              <TableCell>
                                <Badge 
                                  className={
                              ...
                              <TableCell>{referral.lastPayment || 'N/A'}</TableCell>
                              <TableCell>
                                <Button 
                                  variant="ghost" 
                                  size="sm" 
                                  className="h-8 w-8 p-0"
                                  onClick={() => {
                                    setSelectedReferral(referral.id === selectedReferral ? null : referral.id);
                                    toggleReferralSelection(referral.id);
                                  }}
                                >
                                  {referral.id === selectedReferral ? <ArrowUp className="h-4 w-4" /> : <ArrowDown className="h-4 w-4" />}
                                </Button>
                              </TableCell>
                            </TableRow>
                                ...
                                <TableCell colSpan={8} className="bg-gray-50">
                                  <div className="p-4 space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                      <div>
                                        <h4 className="text-sm font-semibold mb-2">Commission History</h4>
                                        <div className="space-y-2 text-sm">
                                          <div className="flex justify-between">
                                            <span>June 2023</span>
                                            <span className="text-green-600">${referral.commission}</span>
                                          </div>
                                          {referral.totalEarned > referral.commission && (
                                            <div className="flex justify-between">
                                              <span>May 2023</span>
                                              <span className="text-green-600">${referral.commission}</span>
                                            </div>
                                          )}
                                          {referral.totalEarned > referral.commission * 2 && (
                                            <div className="flex justify-between">
                                    ...
                                    {referral.status === 'active' && (
                                      <div className="pt-2 border-t border-gray-200">
                                        <h4 className="text-sm font-semibold mb-2">Client Satisfaction</h4>
                                        <div className="flex">
                                          {Array(5).fill(0).map((_, i) => (
                                            <Award 
                                              key={i} 
                                          ...
                                          <span className="ml-2 text-sm">4.0/5.0</span>
                                        </div>
                                      </div>
                                    )}
                                  </div>
