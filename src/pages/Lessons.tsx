...
import { BookOpen, ArrowRight, Clock, Award, Brain, History, ArrowLeft } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { formatDistanceToNow } from "date-fns";
import { Skeleton } from "@/components/ui/skeleton";
import { IS_DEVELOPMENT, FEATURES } from "@/config/env";
...
const Lessons: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAvailableSkills = async () => {
      ...
      try {
        // In production, we'll fetch from the actual database
        ...
        if (data && data.length > 0) {
          // Fetch student's mastery levels for these skills
          const { data: masteryData, error: masteryError } = await (supabase
            .from('student_skills')
            .select('skill_id, mastery')
            .eq('student_id', session.user.id) as any);
            
          if (masteryError) throw masteryError;
          
          // Map the data to include mastery from the student_skills table
          const skillsWithMastery = data.map(skill => {
            const studentSkill = (masteryData as any[])?.find(s => s.skill_id === skill.id);
            return {
              ...skill,
              mastery: studentSkill ? Math.round(studentSkill.mastery * 100) : 0
            };
          });
          ...
          setAvailableSkills([
            { id: "chain", name: "Chain Rule", mastery: 32, description: "Understanding how to differentiate composite functions" },
            { id: "implicit", name: "Implicit Differentiation", mastery: 47, description: "Finding derivatives when variables are related implicitly" },
            { id: "limits", name: "Limits", mastery: 65, description: "Exploring behavior of functions as they approach specific values" },
            { id: "integration", name: "Integration", mastery: 58, description: "Basic techniques of integration, including substitution and parts" }
          ]);
        }
      } catch (error) {
    ...
    const fetchPastLessons = async () => {
      ...
      try {
        ...
        const transformedLessons = data.map((lesson: any) => {
          const matchingSkill = availableSkills.find(s => s.id === lesson.skill_id);
          return {
            id: lesson.id,
            title: lesson.title || (matchingSkill?.name ? `Lesson on ${matchingSkill.name}` : 'Untitled Lesson'),
    ...
    return (
      <Tabs defaultValue="history">
        <TabsList className="grid w-full max-w-md grid-cols-2">
          ...
          <TabsTrigger value="available" ref={availableTabRef}>Available Lessons</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="history" className="pt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Review and continue working with lessons you've previously completed
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoadingHistory ? (
                ...
                <div className="text-center py-4">
                  <p className="text-gray-500">No lesson history found. Start learning!</p>
                  <Button 
                    variant="outline" 
                    className="mt-4"
                    onClick={() => {
                  ...
                  <Card key={skill.id} className="overflow-hidden">
                    ...
                    <CardContent>
                      <p className="text-sm text-gray-600 mb-4">
                        {skill.description || `Enhance your understanding of ${skill.name} through personalized micro-lessons.`}
                      </p>
                      <Button 
                        onClick={() => handleStartLesson(skill.id, skill.name)}
                        className="w-full"
                      >
                        Start Lesson
                      </Button>
                  ...
                  <CardContent className="pt-4">
                    <ul className="space-y-2 text-sm">
                      <li className="flex">
                        ...
                        <span>Regular practice improves retention by up to 75%</span>
                      </li>
                      <li className="flex">
                        <ArrowRight className="h-4 w-4 mr-2 text-brightpair mt-0.5" />
                        <span>Review completed lessons to reinforce knowledge</span>
                      </li>
                      <li className="flex">
                        <ArrowRight className="h-4 w-4 mr-2 text-brightpair mt-0.5" />
      ...
      {!isLessonStarted ? (
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Button 
              variant="default"
              onClick={() => handleLessonComplete(Math.floor(Math.random() * 40) + 60)} // For demo purposes
              className="flex items-center"
            >
              Complete Lesson
            </Button>
          ...
          <div className="p-4 bg-white rounded-lg shadow">
            {lessonQuery.isLoading ? (
              ...
              <div className="text-center py-8">
                <p className="text-xl text-gray-500">No lesson data available</p>
                <Button 
                  variant="outline" 
                  className="mt-4"
                  onClick={() => setIsLessonStarted(false)}
