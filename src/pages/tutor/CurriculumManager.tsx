  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      {curricula.length === 0
        ? (
          <p>
            No curricula created yet. Click "Create Curriculum" to build one.
          </p>
        )
        : (
          curricula.map((cur) => (
            <Card key={cur.id} className="hover:shadow-md transition-shadow">
              <CardHeader>
                ...
                <p className="text-sm text-gray-500">
                  {students[cur.student_id] || cur.student_id} —{" "}
                  {cur.goals.length} goals, {cur.topics.length} topics
                </p>
              </CardHeader>
              <CardContent>
                <Button
                  variant="outline"
                  onClick={() =>
                    navigate("/curriculum?student=" + cur.student_id)}
                >
                  Edit Curriculum
                </Button>
              </CardContent>
      ...
      <Button
        className="bg-brightpair text-white"
